# External Tools
chmod -R 0755 $TMPDIR/volume-key-selector/tools
...
chooseport_legacy() {
  ...
  local error=false
  while true; do
    timeout 0 $TMPDIR/volume-key-selector/tools/$ARCH32/keycheck
    timeout $delay $TMPDIR/volume-key-selector/tools/$ARCH32/keycheck
    local sel=$?
    if [ $sel -eq 42 ]; then
      return 0
    elif [ $sel -eq 41 ]; then
